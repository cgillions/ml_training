...
for trial_id, trial_data in cursor.fetchall():
    ...
    while data_covered < line_count:
        ...
        length = (section_matrix[len(section_matrix)-1][0] - section_matrix[0][0]) / 1000
        step = (section_matrix[1][0] - section_matrix[0][0]) / 1000

        # Compute the 13 mel-frequency cepstral coefficients for the x axis.
        mfccX = mfcc(
            np.asarray([row[1] for row in section_matrix]),
            winlen=length,
            winstep=step,
            nfft=800000
        )
        ...
        mfccY = mfcc(
            np.asarray([row[2] for row in section_matrix]),
            winlen=length,
            winstep=step,
            nfft=800000
        )
        ...
        mfccZ = mfcc(
            np.asarray([row[3] for row in section_matrix]),
            winlen=length,
            winstep=step,
            nfft=800000
        )
        ...
        cursor.execute("""
                        INSERT INTO
                        public."Featureset_2"
                        ("meanXYZ", "stdXYZ", "mfccX", "mfccY", "mfccZ", trial_id)
                        VALUES (%s, %s, %s, %s, %s, %s);
                        """, (means[1:4].tolist(), stds[1:4].tolist(),
                              mfccX.tolist(), mfccY.tolist(), mfccZ.tolist(),
                              trial_id))
